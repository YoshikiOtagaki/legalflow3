...
import { TimesheetEntry, TimesheetListResponse, TimesheetFilters, TimesheetSummary, Timer } from '@/types/timesheet'
...
export function useTimesheet(filters: TimesheetFilters = {}) {
  ...
  const fetchEntries = async (newFilters: TimesheetFilters = {}) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/timesheet-entries?${queryParams}`, {
        headers: {
  ...
  const createEntry = async (entryData: Omit<TimesheetEntry, 'id' | 'createdAt' | 'updatedAt' | 'userId'>) => {
    if (!accessToken) return
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'タイムシートエントリの作成に失敗しました')
      }
  ...
  const updateEntry = async (id: string, entryData: Partial<Omit<TimesheetEntry, 'id' | 'createdAt' | 'updatedAt' | 'userId'>>) => {
    if (!accessToken) return
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.message || 'タイムシートエントリの更新に失敗しました')
      }
  ...
  const deleteEntry = async (id: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.message || 'タイムシートエントリの削除に失敗しました')
      }
...
export function useTimer() {
  ...
  const startTimer = async (caseId: string | null, description: string, tags: string[] = []) => {
    if (!accessToken) return
  ...
  const stopTimer = async (timerId: string) => {
    ...
    try {
      ...
      setIsRunning(false)
      setTimers(prev => prev.map(timer => timer.id === timerId ? updatedTimer : timer))
      return updatedTimer
    } catch (err) {
      setError(err instanceof Error ? err.message : 'エラーが発生しました')
      throw err
  ...
  const pauseTimer = async (timerId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.message || 'タイマーの一時停止に失敗しました')
      }

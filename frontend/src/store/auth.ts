  ...
  login: async (credentials) => {
    ...
    try {
      ...
      if (error.name === 'NotAuthorizedException') {
        ...
        errorMessage = 'アカウントが確認されていません。メールの確認コードを入力してください。';
      } else if (error.name === 'UserNotFoundException') {
        errorMessage = 'このメールアドレスは登録されていません。';
      } else if (error.name === 'TooManyRequestsException') {
        errorMessage = '試行回数が上限に達しました。しばらく時間をおいてから再度お試しください。';
      }
  ...
  register: async (userData) => {
    ...
    try {
      ...
      if (error.name === 'InvalidPasswordException') {
        errorMessage = 'パスワードがポリシーに適合しません。大文字、小文字、数字、記号を含む8文字以上のパスワードを入力してください。';
      } else if (error.name === 'UsernameExistsException') {
        errorMessage = 'このメールアドレスは既に登録されています。';
      } else if (error.name === 'InvalidParameterException') {
        errorMessage = '入力された情報に問題があります。メールアドレスとパスワードを確認してください。';
